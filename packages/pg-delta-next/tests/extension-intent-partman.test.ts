...
describe("extension-intent: pg_partman managed partitions are not dropped (CLI-1555)", () => {
  test(
    "handler-aware extraction + resolveView stop the destructive drop, parent survives",
    async () => {
  ...
  test(
    "the managed plan is proof-clean and preserves child rows (data-preservation)",
    async () => {
      const cluster = await supabaseCluster();
  ...
  test(
    "extension handlers capture on the SAME snapshot as core extraction (coherence)",
    async () => {
